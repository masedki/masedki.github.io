...
### Partie 1. Détection des points aberrants en étudiant la variation des coefficients de régression

Nous allons explorer et étudier le jeu de données [pollution](http://masedki.github.io/pollution.rda). 
...
Les 15 variables explicatives de l'étude sont détaillées [ici](http://masedki.github.io/Pollution-names.txt).
...
`polltrain` et test `polltest` de tailles respectives sont $40$ et $20$. Pour garantir une reproductibilité 
des résultats, on propose fixer le générateur à `119` comme suit 
```{r, eval=FALSE}
set.seed(119)
...
```
<!-- Penser à fixer le générateur de nombres aléatoires pour garantir la reproductibilité des résultats.  -->

Les nuages de points visualisés précédemment sont des simples représentations deux à deux `(covariable, réponse)`. 
...
d. Visualiser l'évoltion la variation de chaque coefficient de régression à l'aide de la fonction `plot()` et repérer à l'aide de la fonction
`indentify` les indices des observationsqui impliquent des variations extrèmes dans le coefficient de régression.
...
f. À partir d'un nouveau jeu de données `polltrain2` que nous allons créer en éliminant dans `polltrain`, les points identifiés dans la question précédente, sélectionner le meilleur modèle au sens du $AIC$, $BIC$, $Cp$ et 
$R^2$^[La recherche du meilleur modèle sera exhaustive en utilisant le package `leaps`. Attention: le critère $AIC$ ne fait pas partie des critères calculés par le package `leaps`].

g. À l'aide d'une validation croisée 2-folds, sélectionner le meilleur modèle de prédiction du jeu de données `polltest`.
